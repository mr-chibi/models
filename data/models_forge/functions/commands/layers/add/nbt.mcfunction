...
execute as @e[type=minecraft:block_display,distance=0..5,limit=1,sort=nearest,tag=forge_layer,tag=!forge_layer_added] at @s run data modify entity @s transformation.left_rotation set from entity @e[type=minecraft:marker,tag=forge_layer,limit=1,distance=0..2,sort=nearest] data.layering[0].transformation.left_rotation
