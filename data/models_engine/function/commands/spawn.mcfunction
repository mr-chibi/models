#
execute if score @s forge_use_spawnegg matches 1 as @e[type=minecraft:item,nbt={Item:{components:{"minecraft:custom_data":{Tags:["forge_save_model"]}}, id: "minecraft:allay_spawn_egg"}},limit=1,distance=0..5,sort=nearest] at @s align xyz unless block ~ ~-1 ~ minecraft:air run summon minecraft:marker ~ ~ ~ {Tags:["forge_paste_compiler"]}
...
# For Creative (functionallity):
execute if score @s[tag=holding_saved_forge_spawn_egg,gamemode=creative] forge_use_spawnegg matches 1.. run trigger model_paste
execute if score @s[tag=holding_saved_forge_spawn_egg,gamemode=creative] forge_drop_spawnegg matches 1.. run trigger model_paste

# For Suvival (functionallity):
execute if score @s[tag=holding_saved_forge_spawn_egg,gamemode=survival] forge_drop_spawnegg matches 1.. run trigger model_paste
...
execute if score @s[gamemode=survival] forge_use_spawnegg matches 1.. run tellraw @s [{"text": "[", "color": "white"},{"text": "Mr_Chibis Models", "color":"#632FF1"}, {"text": "] ", "color": "white"}, {"text": "Removed model entity! If you're in survival mode, please throw the eggs to spawn.", "color": "red"}]
execute if score @s[gamemode=survival] forge_use_spawnegg matches 1.. as @e[type=minecraft:marker,tag=forge_paste_compiler,limit=1,sort=nearest] run kill @s[distance=0..5]
