...
execute unless entity @e[type=block_display,tag=triad_pallete] run tellraw @s [{"text": "[", "color": "white"},{"text": "Mr_Chibis Models", "color":"#632FF1"}, {"text": "] ", "color": "white"},{"text":"Please, stand 1-5 blocks near a model to update primary pallete!", "color": "red"}]
...
execute as @s align xyz anchored eyes positioned ^ ^ ^2 as @e[type=block_display,tag=triad_pallete,sort=nearest] at @s if entity @p[distance=0..5] run data modify entity @s block_state.Name set string entity @p[scores={model_triad_pallete=1}] Inventory[{Slot:-106b}].id
